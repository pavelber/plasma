...
c                        re-calculate PCD response from hv-points of input file to hv-points of present computation 	 
      tf= strt
  1   ti= tf  
      tf= ti+ tstep  
      ...
      do La = 1, LaMx     ! # of zone; 1=BS, 2=Core, 3=Halo     
     ...
     +           ZC(1,La), ZC(2,La),ZC(3,La),ZC(4,La)   
        ...
        write(30+La,'(f7.3, f7.4, f7.0, e8.2, e10.3, 5f9.4, 4e10.3,      ! files "BSinfo.dat", "CoreInfo.dat", "HaloInfo.dat"      
...
c                      "SpInEf(La,iv)" is needed for computation of Win, Wab, WphI, WiRR at t= ti" for d02 of POPs towards "tf".
      ...
      CALL PowYie()   ! Compute Spectral Power from XBR, frames of TREX [file (181], PCD powers [file (59]
...
c                                      to be first used in Scenario of NEXT "ti".
      ...
      open(116, file= 'ZPopsFe_BS.dat')    ! population(t) of all ionization stages in BS
      open(117, file= 'ZPopsFe_Core.dat')  !                                           Core
      open(118, file= 'ZPopsFe_Halo.dat')  !                                           Halo
      do La= 1, LaMx
     ...
     +enNi     DenMn    bp    bcKeV   bwKeV'  
      ...
      open (541, file='PIR_Fe_BS.dat')
      open (542, file='PIR_Fe_Core.dat')
      open (543, file='PIR_Fe_Halo.dat')
      do La= 1, LaMx
     ...
     +e   Li-like   He-like    H-like'
      ...
      open (47, file='CoreLineInfo.dat')
      open (48, file='HaloLineInfo.dat')
      do k7= 1, LaMx
     ...
     +eFr4     YieFr5     YieFr6     YieFr7     YieFr8    tIntegY'    
      ...
      open(403, file= 'HALOemiAbso.dat')  ! Emi and Abso details in Halo
...
c     +                          nX, i, nuQS(i,nX), nuAS(i,nX), PI(i,nX)
          if(nSS.ne.i) then
             ...
             write(*,'(a50)')      'Re-Compile mo1code3.for'
...
c       write(*,'(4i5, e14.6)') iSS1,iQS1, iSS2,iQS2, AIw  
 ...
 88     close(nFi)
      ...
      read(14,*) tPo      ! t-points of broken-line scenario; 10 t-points.
      read(14,'(a9)') comme  
      ...
      read(14,*) R1t  ! radius [cm] of BSs  in "ntp" t-points
      read(14,*) R2t  ! radius [cm] of core in "ntp" t-points
      read(14,*) R3t  ! radius [cm] of halo in "ntp" t-points
      read(14,'(a9)') comme

      do iw = 1, ntp 
         distBS = R2t(iw)*(FoPi/3./nuBSst(iw))**0.3333  ! [cm] mean dist betw BSs taken as cub root of Core Volume per BS
...
c      write(*,'(a22,f11.4,a4)') ' last line has hvC =', hvC(linM4),'eV.' 
	...
	use mo1code3          ! With v-deviation from "hvFine" the resolution decreases monotonicaly to
...
c  PCD with 40mils Kapton filter
     ...
     +          itL= it
      ...
      SUBROUTINE EmiAbso()  ! For La known on CAL, Compu this-La emissivity "EmTot(La,hv)" [W/cc/sr/eV] and 
      use mo1code3          ! absorption coef corrected for stimulated emission"AbTot(La,hv)" [1/cm]            
...
c  FB EMISSION due to RR onto ions of current nX:  (SS=j+1,kf) + e  --> (j,k) + hv   
     ...
     +                    (g0(kf,nX)* FreE* 1.02199815d6)  ! (39); 1.022 MeV== 2mc^2, see mo1code3.for 9 digits 
  ...
  7        continue  
     ...
     +        absoBF(iv), absoBB(iv), absoFF(iv), abTot(La,iv) 
      ...
      use mo1code3          ! "SpInEf(La,hv)" is computed for one RP of each La using one typical LOS     
      implicit none
      real(8) Exte1f, Exte1, Self1, Exte2a, Exte2b, Self2, Exte3a,      
     +                Exte3b, Self3,  ab1, ab2, ab3, ta1, ta2, ta3,  
     +                So1, So2, So3, RP3, dR3, R4, R5, CoPa           
	do iv= 1, nvM
        ab1 = abTot(1,iv)   ! in-BS absorption coef (corrected for induced emission) [1/cm]
        ta1 = ab1*R1  	
        So1 = zero                             
        if(abs(ab1) .gt. 1.d-12) So1 = emTot(1,iv)/ab1   ! [W/eV/sr/cm2]. Excluded /0; notice that "ab1" can be negative (POPs inversion)
c                                                          but "SpIn" remains > 0 because ab1 < 0 makes [1-dexp] < 0 	 
        ab2 = abTot(2,iv)   ! core around BS
        ta2 = ab2*R2  	
        So2 = zero                             
        if(abs(ab2) .gt. 1.d-12) So2 = emTot(2,iv)/ab2   ! [W/eV/sr/cm2]. Excluded /0; notice that "ab2" can be negative (POPs inversion)
c                                                          but "SpIn" remains > 0 because ab2 < 0 makes [1-dexp] < 0 	 
        ab3 = abTot(3,iv)   ! Halo around the core   
        ta3 = ab3*(R3-R2)  
        So3 = zero                             
        if(abs(ab3) .gt. 1.d-12) So3 = emTot(3,iv)/ab3

********************     La=1 is BS.  No perfect sphericity, therefore we express "SpInEf(La,iv)" using 1 LOS.
        Exte1f=                       ! Due-to-Halo part of "External" term in solution of Eq. of Rad Tranf (ERT) 
     +          So3*(one-dexp(-ta3))  ! from-Halo typical SpIn [W/eV/sr/cm2] towards BS, on Halo/Core boundary; equal from all diractions to BS. 
        Exte1 =                       ! non-BS term in solution of ERT for BS
     +          Exte1f * dexp(-ta2)   ! from-Halo SpIn attenuated in the core on Omega-mean LOS path to BS; 
c                                       R2 is OMEGA-AVERAGE in-Core path of LOS from Halo to BS
...
c                                    Then, contrib of this BS EffSpIn(Core) is about 
     ...
     +           (FoPi*(distBS/2.)**2)      ! Spheric surf of R= distBS/2 around RP
        ...
        Exte2b= Exte1f *          ! "Due to Halo" part of Eff SpIn (equal from all directions tovards core RP)
     +          dexp(-ab2*R2/2.)  ! attenuated on direction-averaged path to "core RP" r= 0.8*R2 [as "half mass"] 
c                                   BUT Omega-AVERAGE path from Halo to core-RP (=R2) is longer than 
...
*********************** Halo of inner radius R2, outer radius R3

        Self3= So3*(one-dexp(-ta3))   ! "Due-to-Halo-itself" part of Eff SpIn [W/eV/sr/cm2]. ta3 = ab3*(R3-R2) because 
c                                       (R3-R2) is our estimate of OM-MEAN LOS path in Halo to "Halo RP", not Minimal path =(R3-R2)/2.
c                                        Not far from K13, see above but sph layer; also see Fig 3D.

        RP3 = ((R3**3+R2**3)/2.)**0.3333  ! half-mass "r" of spherical layer
        dR3 = RP3 - R2                    ! shortest thru-Halo path from core/Halo interface to Halo RP
        Exte3a= Exte2a* dexp(-ab3*dR3)    ! "Due to BSs" part of Eff SpIn in RP of Halo  
c                                           is ~ same as in the Core but attenuated on half-width of Halo  
        R4 = sqrt(RP3**2 -R2**2)  ! Length of the tangent (=="kasatel'naja") from RP3 to Core     
        R5 = R2*R4/RP3            ! Radius of cross-sections covered by Core for observer in RP of Halo
        CoPa = 0.25*(R5/R4)**2    ! Part of 4pi [sr] covered by the Core for observer in RP of Halo   
c       if(iv.eq.1) write(*,'(/a16, f7.4/)') 'CoPa =', CoPa   

        Exte3b= So2*(one-dexp(-ta2))  ! Due-to-Core along typical LOS 
     +             *CoPa              ! but reduced by "CoPa" 
     +             * dexp(-ab3*dR3)   ! attenuated in Halo on typical path to its RP

        SpInEf(3,iv)= Self3+ Exte3a+ Exte3b ! La=3 is Halo; SpIn[W/eV/sr/cm2], solution of the RadTrans equation: three terms 

        if(ti.LT.tiInf .and. tiInf.LE.tf) then   ! at print-info time "tiInf" print in "EffSpIns.dat"
          if(iv.eq.1) write(61,'(a162)') ' hvEV       BS      SelfBS    
     + ExteToBS     Core     SelfCore   BSsToCor  HaloToCor    Halo     
     + SelfHalo  CoreToHalo  BSsToHalo   abRbs     abRcore    abR3mR2'
     ...
     +          SpInEf(1,iv), Self1, Exte1,                              ! W/eV/sr/cm2 
     +          SpInEf(2,iv), Self2, Exte2a, Exte2b, 
     +          SpInEf(3,iv), Self3, Exte3b, Exte3a,  ta1, ta2, ta3         
        endif
      enddo   ! iv
      Return		  
      END     ! 'EffSpIn' subr
...
c                          At 8 time points (prescribed in "Params2.inp") the "RadPow" is used  
c                          in computation of 8 frames of TREX [J/keV/sr] and
c						 in computation of full-time radiation yield [J/keV/sr];from XBR  see print (181. 
      use mo1code3       
     ...
     +        Pow5, Pw10, Pw40, FuPow 

*****************  Side-On SpIn.[W/eV/sr/cm2] from x-bright 3-zone sphere is computed using 2 LOSs 
...
******             2nd LOS is periphery (outside core) LOS at y= (R2+R3)/2, see (II.20) 
      ...
      pathCA = sqrt(R3**2 - yHOT**2) - pathCO/2. ! HOT LOS path thru Halo

      Lper = 2.*sqrt(R3**2 - 0.25*(R3+R2)**2)                  ! in-Halo path of cold LOS chosen at y=(R2+R3)/2, (II.16)
      ...
      do iv= 1, nvM 
         ab3= abTot(3,iv)                             ! [1/cm];  La=3 is Halo;  
         So3= zero                             
         if(abs(ab3).gt.1.d-12) So3= emTot(3,iv)/ab3  ! W/cm2/sr/eV = [W/cc/sr/eV] / [1/cm]; excluded /0
c                                                       Note: ab3 (thus So3) can be <0 via POPs inversion; but "SpIn" remains >0 because [1-exp...]<0
         tau3p = ab3*Lper                             ! opacity along full path of cold (Halo-only) LOS
         PeriSpIn(iv)= So3*(one - dexp(-tau3p))       ! [W/eV/sr/cm2] Out from Halo along cold LOS, see (II.20)
...
c  Typical Hot (thru-core) LOS is that at y = R2*(sqrt(5)/3) = 0.7454 R2 which provides opti-thin limit, 
         ...
         So1= zero                             
         if(abs(ab1).gt.1.d-12) So1= emTot(1,iv)/ab1  ! W/cm2/sr/eV = [W/cc/sr/eV] / [1/cm]; excluded /0

         ab2= abTot(2,iv)                             ! La=2 is core
         So2= zero                             
         if(abs(ab2).gt.1.d-12) So2= emTot(2,iv)/ab2	

         tau3= ab3* pathCA               ! opacity on HOT LOS path in Halo (far side)
         SpIn1 = So3*(one- dexp(-tau3))  ! [W/eV/sr/cm2] from far-from-detector side of Halo into core, see (13)
	    
         tau2= ab2* pathCO
         tau1= ab1* pathBS   ! one BS  
         SpIn2=                               ! [W/eV/sr/cm2] from core to Halo towards detector along Hot LOS, see (15)
     ...
     +        + So1*(1.-dexp(-tau1)) *wBS     ! see (15) and (8); this is added by BSs, including probability to catch 1 BS   
         ...
         HotSpIn(iv)=                       ! [W/eV/sr/cm2] from Halo to TREX along hot LOS, see (17)
     +               SpIn2*dexp(-tau3) +    ! from Core, attenuated in Halo 								
     +               So3*(one-dexp(-tau3))  ! added by Halo

         RadPow(iv) = HotSpIn(iv)*pin*R2**2 + PeriSpIn(iv)*Aper  ! [W/eV/sr] from the x-bright 3-zone sphere  
      enddo  ! iv

c     Signals of 3 PCDs CALIBRATED to FULL [4pi sr] radiation power from 1 RBR. 

      Pow5 = zero  ! at-this-t Power [W] shown by PCD with  5 mils Kapton filter
      Pw10 = zero  !                                       10
      Pw40 = zero  !                                       40   
      FuPow= zero	 ! Full Radiation Power from 1 XBR 
...
Collect full-time radiation yield [J/eV/sr] since t0 till StopTime  
     ...
     +            FrYie(iv,1)*1.e3, FrYie(iv,2)*1.e3, FrYie(iv,3)*1.e3,  ! [J/keV/sr] Energy radiated by XBR during one frame. Each "FrYie(iv) is
     +            FrYie(iv,4)*1.e3, FrYie(iv,5)*1.e3, FrYie(iv,6)*1.e3,  !          . convolved with instrumental gaussian in [hvCon1, hvCon2] interval   
     +            FrYie(iv,7)*1.e3, FrYie(iv,8)*1.e3,                    !            "*1.e3" converts [J/eV/sr] to [J/keV/sr]
     +                              SpeY(iv)*1.e3                        ! [J/keV/sr] Radiation Yield gathered since t0 till "StopTime"; convolved with instrumental function.  
c     +           , HotSpIn(iv),                                         ! [W/eV/sr/cm2] from Halo to TREX along hot LOS in the last frame, see (III.15)
c     +            PeriSpIn(iv),                                         ! [W/eV/sr/cm2] Out from Halo along cold LOS in the last frame, see (III.20)
c     +              RadPow(iv),                                         ! [W/eV/sr] Radiation Power from the x-bright 3-zone sphere in the last frame  
c     +            pin*R2**2, Aper                                       ! [cm2]                            
        enddo                                                          
      endif  
      ...
      use mo1code3         ! using params, W's, POPs  of t=ti [these POPs are POPi(k,nX,La)]. 
...
Calculate POPZ(SS) of nX passed thru d02  
     ...
     +                         POPZ(HSS(1)  ,1,La), POPZ(HSS(1)+1,1,La)
      ...
      use mo1code3        ! ionization energy of ions (PIR) in the Ion-Sphere approach           
      implicit none                                    
      real(8) SpheR       
     ...
     +                          PIR(HSS(1)  ,1,La)/PI(HSS(1)  ,1) 
      ...
      use mo1code3        ! Also compu "LvLorW(La,nX,k)" [eV] that is life-time-limited "LvJW(La,nX,k)"; 
      implicit none  
...
c                type "from 2p2 to 1s2p" the line width "FWevLor(li,La)" became non-Zero .
      ...
      use mo1code3        ! CUT ELs with BE < 0, i.e, load their POP in next GrSt
      implicit none      
...
CUT-off block. On this t-step some ELs could get "BE < 1.d-3". I move POP of these ELs to GrSt of next SS         
     ...
     +      POPi(nuGS(kiSS(k,nX)+1, nX), nX,La ) + POPi(k,nX,La)  ! Load this POP(k) in GS of next SS
      ...
      use mo1code3     ! In each W-matrix, first index shows INITIAL EL of the transition.
      implicit none
      integer  kAI     ! mw, 
...
******  RR probability WRR(kf,k):  (j+1, kf) + e --> (j,k) + hv 
            ...
            SiRR= SiPhi * g0(k,nX)* hvw**2 / g0(kf,nX)/eeV/1.02199815d6  ! (39); 1.022 MeV== 2mc^2, see mo1code3.for 9 digits 
...
c                                                                           EAI(kAI,k) + DPI(j-1,nX,La). Griem3 p.178  
  ...
  7     continue
     ...
     +                                   EED2 = bp(La)/bw(La)	            ! 1/eV
      ...
      use mo1code3                  ! note: came here due to "bra(k,kf) > 0.5", see "WI(k,kf)=" loop; "bra" =0/1 is given in "intro" 
      implicit none
     ...
     +     Cix(k,kf,nX)*yw/xw  + Dix(k,kf,nX)*yw/xw/xw
      ...
      if(SigInz .GT. SigMax(La)) SigInz= SigMax(La)  ! (i)  Bad fit to from-FAC points can cause huge SigInz.
c                 	                                 (ii) We assume scaling of from-FAC ioniz cross-secs over eeV/BEk 
c                                                    !      but it can cause huge SigInz (if BEk is small), 
c        therefore I restrict "SigInz", "SigPhi". No need in restricting recombination rates because expressed via SigInz, SigPhi  
      END 
      ...
      use mo1code3                  ! here k,kf convenient for calling ioniz'   
	...
	real(8) function SigPhi(hv)  ! cm2, eV;  PhotoIonz Cross-Sec.   Notations:  (j,k) + hv --> (j+1,kf) + e
      use mo1code3                 ! function "SigPhi" for nX, k, kf, BEk from call
	implicit none      
	real(8) hv, xw
...
c                              SigPhi= 7.9d-18* (BEk/hv)**3 /HSS(nX)**2  ! Kramers;  see BlackFold(42); single-e, nucl charge
      ...
      if(SigPhi.GT.SigMax(La)) SigPhi= SigMax(La)  ! (i)  Bad fit to from-FAC points can cause huge SigInz.
c                 	                               (ii) We assume scaling of from-FAC ioniz cross-secs over eeV/BEk 
...
c        therefore I restrict "SigInz", "SigPhi". No need in restricting recombination rates because expressed via SigInz, SigPhi  
	...
	real(8) ev, V, EED, SigExc       
      ...
      use mo1code3                 ! DE== E(kf,nX) - E(k,nX)
      implicit none
      ...
      END                            ! otherwise no Saha-Boltzmann limit
      ...
      implicit none
      integer Me
      real(8) eeV, As, Bs, Cs, Ds, Es, X, X2, Sig, Gaunt,
     + ALPHA, F, XN, E1 ! Vstavil Fs VB
      As= Ax(k,kf,nX)
      Bs= Bx(k,kf,nX)
      Cs= Cx(k,kf,nX)
      Ds= Dx(k,kf,nX)
     ...
     &         Ds*DEXP(-(XN-1.000D0)**2*E1)) / X
      ...
      if(Sig .LT. zero      ) Sig= zero        ! Bad fits to FAC points may have Sig < 0  
      ...
      use mo1code3                         ! at hvIn1 <= hv <= hvIn2	        
      implicit none  
      integer i1 
...
113   U=X-XX(1)
     ...
     1VV*VV))
      ...
      use mo1code3                       ! has FWHM= 2*sqrt[ln(2)]*GaPa= 1.66510922*GaPa;  Vains, p.248; Griem3 p.54
      ...
      use mo1code3       ! matrix PM(k,kf) and EL de-population rate Wout(k)  
      implicit none 
...
c           PAUSE
     ...
     +	   'My STOP:  POP(k)*PM(k,kf) > 10^17; see "Happened.dat"' 
      ...
      use mo1code3           ! also nuBSs, R1, R2, R3 included in this subr for simplicity 
      implicit none
      ...
      R3 = R3t(i)+ (R3t(i+1)-R3t(i))*frac  !  cm, radius of Halo
      CeR(1)= R1
      CeR(2)= R2
      CeR(3)= R3
...
c      Tion(La) = Te(La)                                       ! Ti [eV]. However, I don't compute and don't use "Ti" in this code   
c                          because the assumption of Ti=Te isn't reasonable for n z-pinches; the e-energy eq cannot be used
c                          because of uncertain radiative loss/gain of a zone in many-zone plasma. One the other hand, 
c                          when operating Te and u3D [thermal+hydro], there remains only one point where I really need Ti(t),

    ...
    with open(spec_num_file, "rb") as inf:
        for line in inf:
            if line.startswith('"'):
                ...
                conf1_low = format_configuration(nist_strip(parts[6]))
                conf2_low = nist_strip(parts[7])
                conf1_up = format_configuration(nist_strip(parts[9]))
                conf2_up = nist_strip(parts[10])
                ek = nist_strip(parts[5])
                ei = nist_strip(parts[4])
                wave = nist_strip(parts[0])
                eins = nist_strip(parts[1])
                osc = nist_strip(parts[2])
                if (conf1_low, conf2_low) in config_table[spec_num] and (conf1_up, conf2_up) in config_table[spec_num]:
                    up_level = config_table[spec_num][(conf1_up, conf2_up)]
                    low_level = config_table[spec_num][(conf1_low, conf2_low)]
...
with open(os.path.join(out_dir, "SPECTR.INP"), 'wb') as spectr_inp:
    with open(os.path.join(out_dir, "EXCIT.INP"), 'wb') as exit_inp:
        ...
        for f in i_spectro:
            write_spectr_section(spectr_inp, str(f), configs, os.path.join(in_dir, str(f) + '.csv'))
            section_lines = read_section(str(f), configs, os.path.join(in_dir, str(f) + '.csv'))

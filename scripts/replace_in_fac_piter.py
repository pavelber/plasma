...
from lib.nist import get_ionization_energy_ev
...
def read_piter(levels_dir):
    configs_per_num = {}
    for levels_file in listdir(levels_dir):
        configs = {}
        num = os.path.splitext(levels_file)[0]
        configs_per_num[num] = configs
        eVColumn = 3
        with open(os.path.join(levels_dir, levels_file), 'r') as levels_file:
            for line in levels_file:
                if "Limit" in line:
                    continue
                data = line.split()
                config = list(filter(lambda l: '(' not in l, data[0].split('.')))
    ...
    for line in old:
        ...
        if len(data) < 9:
            ...
            if levels_config in current_levels and no_4_in_config(levels_config, max_n) and len(
                ...
                print(energy, energy_str, math.isclose(float_energy , float(energy), rel_tol=1e-9))
            else:
                new.write(line)
                energy_str = energy
...
if len(sys.argv) < 3:
    error('\nUsage: ' + sys.argv[0] + ' directory-with-fac output-directory nmax')
...
piter_dir_name = os.path.join(fac_nums_out_dir, "piter-" + elemnt)

if not os.path.exists(piter_dir_name):
    os.mkdir(piter_dir_name)
    download_piter_levels(elemnt, piter_dir_name, nmax)

piter_levels_per_num = read_piter(piter_dir_name)
...
for fac_dir_name in listdir(fac_nums_dir):
    ...
    if next_sp in piter_levels_per_num:
        next_sp_nist = piter_levels_per_num[next_sp]
    else:
        next_sp_nist = {}
    levels = [piter_levels_per_num[num], next_sp_nist]

    with open(fac_lev, 'r') as fac_lev_file:
        with open(fac_lev_tmp, 'w') as fac_lev_tmp_file:
            old_energy_to_levels[num] = recreate_fac_lev(fac_lev_file, fac_lev_tmp_file, levels, nmax, elemnt,
                                                         int(num))

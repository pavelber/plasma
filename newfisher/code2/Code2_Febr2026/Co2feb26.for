...
c                        initial population ("POPs") of energy levels ("ELs")
  ...
  1   ti= tf  
      ...
      do La = 1, LaMx     ! La is zone number. La=1 is BS, La=2 is Core, La=3 is Capsule     
        CALL SCENARIO()   ! for t= ti compute R, Te, TD, DenI, ZC1, Dene
        write(*,'(a33, i2, e11.4, 2f7.0, e11.4, f10.4)') 
     +          'Zone, R(cm), Te, TD, ne, Z(X) =', 
     +           La, CeR(La), Te(La), TD(La), Dene(La), ZC(1,La)  
     ...
     +                                bp(La), bc(La)/1.d3, bw(La)/1.d3   ! [keV]
        ...
        do nX = 1, 1 ! nXE  ! For Te=Ti, Den(nX,La) of t = ti. Note: only X has lines and DPI   *********** !!!! *********
           CALL LevStW()    ! For these La,nX compu FWHM [eV] of all E-LEVELs "LvJW(La,nX,k)" [eV] 
...
c             Amass = AtMass(nX) *Uam                     ! [g] mass of nX ion
c             uTi   = sqrt(8.*BolEr*Tion(La)/pin/Amass)   ! [cm/s] mean thermal velocity of nX ion in La
c             fwTiKvC(La,nX)= 1.4757*uTi/c                ! my page on thermal and isotropic 3D hydro broadenings                        
c             fwTUKvC(La,nX)= 1.4757*u3D(La)/c
c             FWkVcGAU(La,nX)= sqrt(fwTiKvC(La,nX)**2 +fwTUKvC(La,nX)**2)  ! FWHMgau/hvC of 2 convolved Gaussians.
           FWkVcGAU(La,nX)= 7.7155e-5*sqrt(TD(La)/AtMass(nX))  ! my page on thermal & hydro broadenings, see BlackFold   
        enddo  ! nX
...
c	                  for params of t=ti and POPi(k,nX,La).  Print frames in files (181 - (186 and power detectors in files (187 - 189 	 
      ...
      do La = 1, LaMx	 
      do nX = 1, 1  ! nXE  ! ******** !!!! ****   
cccc         write(*,'(a30, 2i3, e10.2)') 'La, nX, Dene(La) =',           !@#$%*******************
cccc     +                                 La, nX, Dene(La)        
         if(Dene(La).gt.1.e15) then  ! For each (La,nX) compute Ws & PM; run d02 for POPs from "ti" to "tf" with params of "ti".  
c                                    ! Note: ne > 10^15 e/cc excludes inrerruption in NAG d02EAF. For some {Te, dTe/dt, Jv} this NAG subr may work even for lower ne       
cccc                 write(*,'(a30)') 'CALLing Atkins'         !@#$%***********************
               CALL AtKins()  ! It gives "POPf(k,nX,La)" that is POP(tf) to be first used in Scenario of NEXT "ti" as POPi(k,nX,La).
cccc                 write(*,'(a30)') 'Returned from Atkins'   !@#$%***********************
         endif
      enddo   
      enddo   
cccc              write(*,'(a30)') 'Returning to mark 1'   !@#$%***********************
      ...
      do La= 1, LaMx
	   write(30+La,'(a146)')     'tiPS     Rmcm   TeKeV   TDkeV   Dene
     +       Zbar       Z1       Z2     Z3     Z4      Den1      Den2   
     +   Den3      Den4     bp    bcKeV   bwKeV'  
      enddo
      ...
      do k7= 1, LaMx
        write(45+k7,'(a143)')      'Info on FWHM [eV] of spectral lines.
     +    Note: 0 in column "Baranger" means that upper level is cut by
     +lowering of continuumg.'
        write(45+k7,'(/a155)') 'hvCeV  ChE SpS  Lambda(A)   Upper        
     +      Upper                 Lower                Lower       Gauss
     +   Baranger  Lorentz   Voigt   PopUpper    flu'
      enddo
      ...
      do La= 1, LaMx
         write(115+La,'(/a114)')   'tfPS   TeKeV    Dene     Den1      P  ! printed just after "AtKins", thus t= "tf". After 
...
c                                       Spectral response of spectrometer_1 [arb.u.].  Spectrograms (spectral yield * response) [arb.u]  
     ...
     +tr6    spectr7    spectr8    SpectrY'
      ...
      open(187, file= 'PowDet1.dat')  ! [TW] and [arb.u.] t-dependent signal of Power Detector #1.  (former PCDs)
      open(188, file= 'PowDet2.dat')  ! [TW[ and [arb.u.] t-dependent signal of Power Detector #2
      open(189, file= 'PowDet3.dat')  ! [TW] and [arb.u.] t-dependent signal of Power Detector #3
      do k = 1, 3
      ...
      do nX= 2, nXE    !  C, He, D                 !@#$%*********************************
        ...
        do i= FSS(nX), HSS(nX)  ! all SpSs of nX, except its NUCL                
          ...
          if(nSS.ne.i) then
             ...
             write(*,'(a50)') 'Re-Compile mo1co2feb.for'
...
c                       Read 'Params1.inp'.
      ...
      do nX= 1, nXE
        read(13,*) AtMass(nX)  ! ion mass [a.u.] for Stark calculation of line widths. 
      enddo					 ! Note: Gauss FWHM of Kr lines is via TD/Mi(Kr)
      read(13,*) empty
...
c                                    Note: must be R1 < dc < R2-R1
      ...
      do j = 1, ntp
c                    write(*,'(2i3, 7f10.4)') j, NCL, R2t(j), R3t(j)	  !@#$%%%%%%%%%%%%%%%%%%%%%%%%% 
     ...
     +                   'R3 <= R2/(1-2/NCL); increase R3' 
      ...
      do j = 1, LaMx 
         read(14,*) W1D       
         do i = 1, ntp 
           TDt(i,j) = W1D(i)  !  t-points of Te [eV] in zone #j
         enddo
      enddo
      read(14,'(a9)') empty
...
***   Re-calculate spectral response of each detector from hv array of input files to hv array of present computation 
     ...
     +            icL= ic
      ...
      use mo1co2feb         ! at t = ti, using all params of t= ti and POPi(EL,nX,La)  
      implicit none          
...
c													  ! DiZ2(La) is sum over all XE of ion density *mean ion charge^2
     ...
     +                     * (one- exp(-hveV/Te(La)))   ! Corrected for stimulated emission in Maxw case. 
      enddo
...
***      SpIn remains > 0 because Abso < 0 causes (Emi/Kap) < 0  and  [1 - exp(-kL)] < 0.
  ...
  1     continue
        ...
        FWevDop= FWkVcGAU(La,nX)*hvC(lw)   ! via TD/Mi
        ...
        if(ti.LT.tiInf .and. tiInf.LE.tf) then           ! at print-time "tiInf" print in "...LineInfo.dat"
           write(45+La,'(f9.3, 2i4, f12.4, i6,a1,a24, i16,a1,a24, f9.3,   
     +            3f9.2, e11.3, f7.3)')  hvC(lw), nX3(lw),  kiSS(kU,nX),  	
     +          eVA/hvC(lw), kU,'=',QSname(kU,nX), kL,'=',QSname(kL,nX), 	
     +          FWkVcGAU(La,nX)*hvC(lw), QMlimit(lw,La), FWevLor(lw,La), 
     +                        VoiFWHM, POPi(kU,nX,La), flu(kL,kU,nX) 
        endif
      enddo   ! lw loop
...
c  FB EMISSION due to RR onto ions of current nX:  (SS=j+1,kf) + e  --> (j,k) + hv   
     ...
     +                    (g0(kf,nX)* FreE* 1.02199815d6)  ! (39); 1.022 MeV== 2mc^2, see mo1co2feb.for 9 digits 
  ...
  7        continue  
     ...
     +        absoBF(iv), absoBB(iv), absoFF(iv), abTot(La,iv) 
      ...
      use mo1co2feb        ! is "SpInEf(La,hv)" [W/eV/sr/cm2], see (II.9), (AI.0). In manual "La" is "b".
      implicit none
...
c                                   probability that BS is inside Vray is (nuBSs-1) *Vray/Vco
...
c                             + factor =', SpInEf(3,iv), Vcap, (2.*pin**2)/(3.*Vcap*Ntet)         !@#$% 
     ...
     +                                    ab3*(R3-R2)        
      ...
      use mo1co2feb       ! This subroutine is CALLed from outside La-loop, thus on entry has La = LaMx+1 	
      implicit none  
...
c     +    write(341,'(//a32, f6.2, a7/)')  '** ti =', ti*1.e12,'ps **' 
      ...
      nCo = 0                  ! number of thru-core towards-target LOSs
      do k = 1, NCL
         rc(k) = drZ*k - hadr  ! [cm ] center of k-th radial zone
         L2    = 0.
         if(rc(k).lt.R2) then	 ! k-th LOS crosses Core
	      nCo= k                        
...
c   Frame 1 from "3 BSs of same total mass" (i.e. empty Core & Capsule) showed that my earlier computations 
c   via "wBS in SpIn(iv,k)" (see "Co2feb2025_Save nov22.for") doesn't provide equality of those Frames,
...
c   I distribute "SpPow(iv)" among thru-Core LOSs according to Vo(k)/Vco (only for t = tiInf):   
      ...
      if((tiInf-tstep/2.) .LE. ti .and. ti.LT. (tiInf+tstep/2.)) then  
          ...
          do k  = 1, nCo    ! only thru-core LOSs
                SpInBSs(iv,k)= (SpPowBSs(iv)*Vo(k)/Vco)/pin/R2**2   ! [W/eV/sr/cm2] 
                SpIn(iv,k) = SpInCC(iv,k) + SpInBSs(iv,k)           ! [W/eV/sr/cm2] from target (Core+Capsule+BSs)
          enddo
          enddo 
      endif      
...
********  denoted "diSpIn(hv, diLOS#)".[W/eV/sr/cm2] & is calculated at ti = "tiInf" using known "SpIn(iv,k)" 
     ...
     +    Respon    SpeArbU'
           ...
           L2= 0.
           if(rdi(i) .LT. R2) L2= 2.*sqrt(R2**2 - rdi(i)**2)  ! [cm] thru-Core path of i-th diagnostic LOS
           L3= sqrt(R3**2 - rdi(i)**2) -L2/2.                 ! [cm] ONE-SIDE thru-Capsule path of i-th "diagnostic" LOS
           if(L3.LT.0.) STOP 'L3 < 0, case 2'
           do iv = 1, nvM		
...
********  at ti = tiInf. To be printed in file "AlongCeLOS.dat". No spectrometer, no spectral response
     ...
     +    k1xL1    k2x2R2   k3xR3mR2'	 
         ...
         wBS= (nuBSs*L2*(2.*R1)**2)/Vco  ! probability to catch a BS by central LOS.  
c                                        The LOS traverses a BS if touches, at least, edge of BS, i,e,
c                                        entier BS is inside volume VLOS= L2*pi*(2.*R1)^2.  
c                           N BSs are randomly distributed in Core of volume Vco,  
c                           then probability to find at least one BS in VLOS is  nuBSs*VLOS/Vco 	
         do iv= 1, nvM
...
******** spectral response of these detectors "Resp(iv,j)",  Instruments # = 6+j =7,8,9.
     ...
     +              	              PowDet*FoPi/1.e12, PDsign(j)  ! [TW], [arb.u.] 
         enddo  ! j
      enddo	  ! iv  
ccccc                       write(*,'(a45)') 'Out from PowYie'     !@#$%********************
      Return 					  								 
      END     ! of 'PowYie subr
      ...
      use mo1co2feb        ! all params, W's, POPs  of t=ti. Note: these POPs are POPi(k,nX,La).  
...
ccccc                      write(*,'(a35, i2)') 'Came to D02EAF, La=', La     !@#$%********************
      CALL D02EAF(tiS, tfS, NST(nX), POP, tolD02, POPdot, WEAF, Nwork,  
     +            ifail) 
ccccc                      write(*,'(a35, i2)') 'Out from D02EAF, La=', La    !@#$%********************
...
c        write(*,'(a35,1pe10.2)') 'Corrected: SumPOPs-1 =', Sum1-one 
      if(abs(Sum1-one).gt. 1.d-14) PAUSE 'insufficientR correction'
...
c                                 Scenario of next "ti" == present "tf" for proper "La" and "XE" 
     ...
     +                         POPZ(HSS(1)  ,1,La), POPZ(HSS(1)+1,1,La)
      ...
      use mo1co2feb       ! ionization energy of ions (PIR) in the Ion-Sphere approach           
      implicit none                                    
      real(8) SpheR       
     ...
     +                          PIR(HSS(1)  ,1,La)/PI(HSS(1)  ,1) 
      ...
      SUBROUTINE LevStW() ! For these La,nX compu J-Stark FWHM of all energy levels "LvJW(La,nX,k)" [eV] following Refs [8,9]. 
      use mo1co2feb       ! "k" is # of energy level 
      implicit none  	 
...
c  the E-lield  unit is   e/a0^2  = 5.14220652e9 V/cm 
     ...
     +                    WSr(nXE+1) /TeAU )  ! IMPO: e-charge = -1 in Jenya's formulary 
      ...
      do k= 1, NST(nX)            ! compute Stark FWHM of all energy levels of La,nX that CALLed "LevStW"
...
c                type "from 2p2 to 1s2p" the line width "FWevLor(li,La)" became non-Zero .
      ...
      QMlimit(li,La) = hBar* ( WoutXE(qU,nX,La)+ WoutXE(qL,nX,La) )  ! eV	! the uncertainty principle "dE*dt > h/2pi"
      ...
      use mo1co2feb       ! CUT ELs with BE < 0, i.e, load their POP in next GrSt
      implicit none      
...
CUT-off block. On this t-step some ELs could get "BE < 1.d-3". I move POP of these ELs to GrSt of next SS         
     ...
     +      POPi(nuGS(kiSS(k,nX)+1, nX), nX,La ) + POPi(k,nX,La)  ! Load this POP(k) in GS of next SS
      ...
      use mo1co2feb    ! In each W-matrix, first index shows INITIAL EL of the transition.
      implicit none
      integer  kAI     ! mw, 
...
******  RR probability WRR(kf,k):  (j+1, kf) + e --> (j,k) + hv 
            ...
            SiRR= SiPhi * g0(k,nX)* hvw**2 / g0(kf,nX)/eeV/1.02199815d6  ! (39); 1.022 MeV== 2mc^2, see mo1co2feb.for 9 digits 
...
c      end  
      ...
      use mo1co2feb                       ! in interval narrower than [minGaLor, maxGaLor] [eV] 	        
      implicit none  
      integer i1 
...
c                                                           FWHM/hv = Ains + Bins* hv[keV] + Cins* (hv[keV])^2
      ...
      use mo1co2feb                      ! where GAuss PArameter "GaPa" == FWHM/1.66510922
      ...
      use mo1co2feb                       ! in interval < [minGaLor, maxGaLor] [eV] 	        
      implicit none  
      integer i1 
      real(8) Simu(nvM), Co(nvM), fwPv, v0, FWins, Sver, dev, InsFunc,    
     +                                      Loren, prF, prL, ArLor   
...
c      close(342)
     ...
     +                                   EED2 = 1./bw(La)	        ! 1/eV, norm to under-shape area=1
      ...
      use mo1co2feb
      implicit none
      real(8) eeV, V, EED, SigInz     ! function "SigInz" requires nX, k, kf and assums KiSS(kf)=KiSS(k)+1       
      ...
      use mo1co2feb                 ! note: came here due to "bra(k,kf) > 0.5", see "WI(k,kf)=" loop; "bra" =0/1 is given in "intro" 
      implicit none
...
c                                                      (ii) Also, bad fit to from-FAC points can cause huge SigInz.
c        therefore I restrict "SigInz", "SigPhi", no need in restricting recombination because expressed via SigInz, SigPhi 
      ...
      use mo1co2feb                 ! here k,kf convenient for calling ioniz'   
	...
	real(8) function SigPhi(hv)  ! cm2, eV;  PhotoIonz Cross-Sec.   Notations:  (j,k) + hv --> (j+1,kf) + e
      use mo1co2feb                ! function "SigPhi" for nX, k, kf, BEk from call
	implicit none      
	real(8) hv, xw
     ...
     +                         xw**(3.5+ Hix(k,kf))   
c                              SigPhi= 7.9d-18* (BEk/hv)**3 /HSS(nX)**2  ! Kramers;  see BlackFold(42); H-like to nucl.
...
c        therefore I restrict "SigInz", "SigPhi". No need in restricting recombination because they are expressed via SigInz, SigPhi 
	...
	real(8) ev, V, EED, SigExc       
      ...
      use mo1co2feb                ! DE== E(kf,nX) - E(k,nX)
      implicit none
...
113   U=X-XX(1)
     ...
     1VV*VV))
      ...
      use mo1co2feb     ! matrix PM(k,kf) and EL de-population rate Wout(k); also dZ/dt  
      implicit none 
...
c              write(*,'(a40,8e9.2)')'R1 diBSs, dc R2=', R1,diBSs, dc, R2    !@#$%************
     ...
     + (II.1) and decrease Nbs.'  
      ...
      enddo
        ...
        Tion(La) = Te(La)                                     !  Ti [eV] rather uncertain but is used only in due-to-ions part of Stark
        TD(La) = TDt(i,La)+ (TDt(i+1,La) - TDt(i,La))*frac    !  Doppler temperature [eV] at "ti" in this La

...
10          continue
      ...
      Dx  = zero 
      Ex  = zero         
      Fx  = zero  ! not used in excit methods 5 and 11 used now VB
...
c     +                                   nX, FSS(nX), HSS(nX), PI(i,nX)
      ...
      if(nuGS(HSS(nX)+1, nX) .ne. Nnu(nX)) PAUSE 'Inconsistency #2'  ! kolichestvo vseh ne AI urovnej +1 ne ravno nomeru yadra
...
c  Read ionization cross-sec coefs from "Inz.inp" and assign "1" to "bra(i,f,XE)" if Yes ioniz channel in "Inz.inp"
  ...
  9     read(nFi,*) iSS1, iQS1, iSS2, iQS2, Axw, Bxw, Cxw, 
     +              Dxw, MePh, Exw, Fxw, Gxw, Hxw, thre    ! ioniz threshold
	  CountInz = CountInz + 1
c       Razdelyaet obichnie urowni i avtoionizacionnie (VB)
...
c        therefore I restrict "SigInz", "SigPhi". No need in restricting recombination because they are expressed via SigInz, SigPhi 
	...
	real(8) ev, V, EED, SigExc       
     ...
     +                                                  nX, k, kf, DE
      ...
      X= eeV/DE      ! Tekushaq energiya w porogowih edinicah VB coment 
      if(X.LT.1.) then
     ...
     +                                                  Gauss, prF
      ...
      do i1= 1, nvM
        v0 = hvV(i1)
        if(v0.LE.hvIns1) cycle   ! hvIns1 - verhnij predel integrirowaniya v konvolyucii
        if(v0.GE.hvIns2) cycle   ! hvIns2 - nizhnij predel integrirowaniya v konvolyucii

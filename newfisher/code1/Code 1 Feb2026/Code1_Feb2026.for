	...
	write(*,'(a53)') 
     +            'tf[ns]  D[mm]   Te[eV]    TD[eV]     i/cc    %2ndCE'  
...
******************  Build the sequence of t-points
      ...
      if(npa.LT.10) npa = 10  ! number of t-points in each (tres(j), tres(j+1)] t-interval. This number includes t= tres(j+1) point  
...
********************  Main "ti'- tf" loop
 ...
 11   continue    ! do next t-step
      ti = tf     ! "tf" of completed (ti-tf] interval becomes "ti" for to-be-done interval.
      ng = ng+1
      tf = tgp(ng)

      La= 2  ! only sphere, no La-loop 	
...
***   Print input params [D,Te,TD,ni, part of Mg in denI (<=1)] on the screen. 
***   I show them at the end of (ti-tf] interval because used them for t-integration of POPs from "ti" to "tf". 
      write(*,'(f8.4,  f7.3,     f9.0, f10.0,   e11.3,  f7.2)') 
     +       tf*1.d9, 2*R2*10.,  Te,   TiD,     DenI,  paMG*100.  							      
...
***   Print [R,Te,TD,ni,niAL,niMG,Zbar,ne] in file MHD.dat. 
***   Relate them to the end of (ti-tf] because used for t-integration up to t=tf. 
      write(32,'(f6.4,     f6.3, f8.0,   2f7.0,  2e10.3, 2f8.3, e11.4)')   
     +           tf*1.e9, 20.*R2, Te,  TiBS, TiD, Den,    ZC,   Dene    ! [ns] [mm] [eV] [i/cc]; TiD is Doppler temperature [eV]
...
c  Print all-XE POPZ for this "La" and t = tf   			
      do  nX = 1, nXE
        ...
        enddo
      enddo
      write(117,'(f9.4, 10e11.4, f7.3, f9.0, f8.0, e11.4)') tf*1.e9, 
     ...
     +  POPZ(12,2,La), POPZ(13,2,La), 20.*R2, Te, TiD, DenI       

      CALL EmiAbso()   ! update this-La EmTot(La,hv) and AbTot(La,hv)
      CALL EffSpInK13() 
      CALL RTeSpIn()   ! Calculate towards-detector Radiation Flux from sphere [W/eV/sr]. 

      do k3= 1, mSpe                ! check all in-Params print-times for prints (60+  
         if(ng .eq. gpEq(k3)) then  ! "gpEq(...)" is NUMBER of t-point at which t = tres(...):  
            CALL Frames(k3)         ! print "SpePow(iv)" in "mSpe" files (60+ .
         endif 
      enddo
      ...
      open(41, file= 'DPIs and POPs.dat')
      ...
      open(117, file='ZPops.dat')  ! (La=2)
         write(117,'(a152)')  'tfNS      AlBe       AlLi       AlHe    
     +  AlH        ALnucl     MgBe       MgLi       MgHe       MgH      
     +  MGnucl     Dmm    TeEV    TDeV      DenI'

       open(21, file='POPs_AL.dat')                                      ! POPs(t) of chosen six AL levels, numbers according to "QSsAL272.inp"  
      write(21,'(a76)') 'tfNS   TeEV        A         B         C        ! Chosen Level## shown in "Params.inp" 
     +   D          E          F'     
     
       open(22, file='POPs_MG.dat')                                      ! POPs(t) of chosen six MG levels, numbers according to "QSsMG272.inp"  
      write(22,'(a76)') 'tfNS   TeEV        A         B         C        ! Chosen Level## shown in "Params.inp" 
     +   D          E          F'     

       open(32, file= 'MHD.dat')
      write(32,'(a78)') 'tfNS   Dmm   TeEV   TiEV   TDeV     niAL      n
     +iMG    ZbarAL  ZbarMG    Dene'  

      open(46, file= 'LineWidth.dat')

      open(61, file= 'SpePowFr1.dat') 
      open(62, file= 'SpePowFr2.dat')  ! FULL-hv-RANGE Radiation Flux [W/eV/sr] from 1 plasma sphere
      open(63, file= 'SpePowFr3.dat')  
      open(64, file= 'SpePowFr4.dat') 
      open(65, file= 'SpePowFr5.dat')    
      open(66, file= 'SpePowFr6.dat') 

      open (452, file= 'EffSpInK13inPrintFr.dat')                   
      write(452,'(a32)')  'hvEV       EffSpIn     kapR' 	     

      open (100,file='Powers.dat')                   
      write(100,'(a79)') 'tfNS   PowWpSr      PwBe      Pow5m      Pow10
     +m     Pow40m     TeKeV      ni'
      ...
      SUBROUTINE Intro()  ! Read input files, print list of ELs, provide ELs p.q.n., A coefs and initial POPs; print this info in "Al,Mg DPI and POPs.dat"  
      use mo1 
      implicit none
...
c  Read 'Params.inp'
	...
	do i = 1, mSpe
      ...
      read(13,*) PoLeNu(1,1), PoLeNu(1,2), PoLeNu(1,3), PoLeNu(1,4),     ! Numbers of 6 levels of Al chosen for printing their POP(t) in files "POPs_AL.dat". 
     +           PoLeNu(1,5), PoLeNu(1,6)             
      read(13,*) PoLeNu(2,1), PoLeNu(2,2), PoLeNu(2,3), PoLeNu(2,4),     ! Numbers of 6 levels of Mg chosen for printing their POP(t) in files "POPs_MG.dat". 
...
*                                = 2 for smooth two-half-Lorentzians approximation of Te(t)
      ...
      read(13,*) NT   ! number of coaxial cylindrical tubes taken for computation of from-sphere SpePow
      close(13)

      write(46,'(a130)') 'Info on Line Broadening (FWHM/hvC) at t = tres
     +(PrFr).  Code omitted lines with PopU < 1.d-8 and lines with hvC <
     + hvSmo.'
...
c  Print common (all-XE) line list into file #30 
  ...
  1     continue
        ...
        StJen = 0.    ! Remains for all spectral lines except resonant K-shell lines.     
c  Approximate expression for Lyman lines and resonant lines of He-like ions, Here for Aluminum and Mg, based on "Interactive Formulary"
        if(hvC(lw).gt. PI(HSS(nX),nX)/2.) then  ! [eV], "PI(H-like)/2" separates resonant K-shell lines from softer lines  
     ...
     +                   (Dene/1.d22)**0.6 *(Te/1.d3)**0.2  
        ...
        FWevLor(lw)= max(FWevNatu, StJen)  ! [eV] Lorentz FWHM; Note: "StJen" =/= 0 only for resonant K-shell lines and their satellites. 
...
c                                                  Note: "BEk < 0" never happens for non-AI k because big DPI cuts it. 
 ...
 90   continue
     ...
     +         absorBB(iv), absorFF(iv), absorBF(iv), abTot(2,iv) 
      ...
      do iv= 1, nvM
         SpInEff(2,iv) = zero    ! La=2 is plasmaa sphere
...
c                                                       ! "SpInEff" remains > 0 because tau < 0 made (1-dexp(-tau) < 0
     ...
     +       write(452,'(f13.4, 7e11.4)') hvV(iv),                      ! "gpEq(j)" is NUMBER of t-point at which tf = tres(j)
     +                              SpInEff(2,iv), tau                  !  W/cm2/sr/eV 
      enddo
      Return		  
      END     ! 'EffSpIn' subr
  ...
  1   continue
      ...
      SUBROUTINE RTeSpIn() ! Analytical solution of Rad.Trans.Eq. along NT parallel LOSs gives "SpePow(iv)" [W/eV/sr].
c                            Present subr works at each t-step because needed for POWER(t) via "SpePow"  
      use mo1 
      implicit none															   
      real(8) ArK(NTmx), Rout(NTmx), rw(NTmx), pathBS(NTmx),  
     +                   ArBS, So2, exw   

      ArBS= pin*R2**2   ! (Plane Round) Area of circle seen to far detectors
      Rout(1) = R2/NT            ! Outer radius of 1st (central) cylinder around central towards-detector LOS, Fig.1
       ArK(1) = pin*Rout(1)**2   ! Area of 1st radial zone (plane belt) 
        rw(1) = Rout(1)/2        ! middle radius of 1st cylinder        
      do k = 2, NT
         Rout(k) = (R2/NT)*k	                       ! Outer radius of k-th cylindric tube 
          ArK(k) = pin* (Rout(k)**2 - Rout(k-1)**2)  ! Area of cross-section of k-th cylindric tube
           rw(k) = (Rout(k) + Rout(k-1))/2.          ! middle radius of k-th cylindric tube
      enddo 
      do k = 1, NT
         pathBS(k)= 2.*sqrt(R2**2 - rw(k)**2)  ! thru-sphere path of k-th LOS; 
      enddo 

      do iv= 1, nvM
         if(abs(abTot(2,iv)) .gt. 1.d-20)  ! exclude "/0." in next line;  La=2 is plasma sphere (BS),  
     +      So2 = emTot(2,iv)/abTot(2,iv)  ! in case of POPs inversion: "abTot" can be < 0  (alfa > 1) but 
c                                            "SpePow" remains > 0 because [1-dexp...] is also negative
         SpePow(iv) = 0.
         do k = 1, NT  ! coaxial cylindrical tubes 
            exw = dexp( -abTot(2,iv)* pathBS(k) )   
            SpePow(iv) = SpePow(iv) + ArK(k)*So2*(1.-exw)   ! [W/eV/sr] from plasma sphere tovards a detector
         enddo 
      enddo   ! iv
      Return
      END     ! of subr 'RTeSpIn"
      ...
      SUBROUTINE Frames(FrNu) ! Print FrNu-th frame of SpePow [W/eV/sr] from seen-to-spectrometer circle
      use mo1                 ! No Doppler shift  
      implicit none
      integer  FrNu   ! serial number of frame  

c  Print "SpePow(iv)" at "FrNu" time [in file (60+ that is "SpePowFr#.dat"]
c  Note:  SpePow(iv) is computed on each t in SUBR "RTeSpIn()" but I moved the print here because 
c         spectrograms are needed only at "mSpe" times "tres(#)", see CALL Frames .

	write(60+FrNu,'(a38)') 'hvEV      iv    SpePow     KapD'  ! "SpePow(hv)" [W/eV/sr] is known in full interval of hv          
      do iv= 1, nvM
         write(60+FrNu, '(f12.2, i7, 9e11.4)') hvV(iv), iv,              ! eV;  
     +                                 SpePow(iv), abTot(2,iv)*2*R2     ! [W/eV/sr] from 1 BS; Opacity along central LOS of BS (La=2), 3rd index = 1 means motionless
      enddo
      close(60+FrNu)

      if(FrNu.LT.mSpe) write(*,'(/a53)')
     +            'tf[ns]  D[mm]   Te[eV]    TD[eV]     i/cc    %2ndCE'  

      if(FrNu .eq. mSpe) then      
             write(*,'(/a50/)') 'Finished the Computation.'  
             STOP 	
      endif
      Return
      END     ! of subr "Frames(FrNu)"
     ...
     +        RespBe, Resp40, Respo5, Resp10        ! Response of four PCDs. 0 <= Respo <=1   
      ...
      PwBe  = zero  ! [W]: v-integral of "SpePow" thru Be-filter in 4pi sr  
...
c    hv-integration of from-BS specific power "SpePow(iv)" [W/eV/sr] . 
c    PCD covers solid angle "OmDet" thus collects PowPCD[W] = OmDet[sr]* vInteg{Response*SpePow}[W/eV/sr];   0 <= Respo <=1 
c    Full (4pi) solid angle is bigger than "OmDet" by factor "4pi/OmDet", thus 
c    full Power[W] = 4pi[sr]* vInteg{Response * SpePow[W/eV/sr]}

         Fpow  = Fpow + SpePow(iv) *dhv          ! [W/sr]; Full-spectrum radiated power, to be used in computation of Ti(t), eq.(2) 
         PwBe  = PwBe + SpePow(iv) *dhv *RespBe    
         Pow5  = Pow5 + SpePow(iv) *dhv *Respo5         
         Pw10  = Pw10 + SpePow(iv) *dhv *Resp10      
         Pw40  = Pw40 + SpePow(iv) *dhv *Resp40         
      enddo  ! iv  

      FpowPr = Fpow
 
      write(100,'(f7.4, 19e11.4)') tf*1.d9, Fpow,                        ! [ns], [W/sr] in full hv range  
     +                             PwBe, Pow5, Pw10, Pw40, Te/1.d3, DenI ! PCD-Be and three PCDs with Kapton filters (5, 10, 40mils)  
     +                                         
...
c  3rd term in eq (Ti-Te)/teiE = dTe/dt + 2*Fpow*FoPi/3/V1/ne/kB  ! [eV/s]; kB==BolJ in [J/eV] 
      TeDotPow  =  2.*Fpow*FoPi/3./Volu(2)/Dene/BolJ              ! [eV/s] per 1 e

      TiBS = Te + teiE* (TeDot + TeDotPow)  ! Ion temperature [eV]   
      if(TiBS .le. 0.)  STOP 'My STOP caused by Ti <= 0'
      if(TiBS .gt. TiD) STOP 'My STOP caused by Ti > TD'
...
Comment on RESTART of dead 'k' level:  when BEp(k,XE,Layer) <0 changes to BE(k)>0, then initial POP(k)==0 is  given proper value in D02
      ...
      CALL PMg0  ! PM(k,kf) and Wout(k,nX,La) for g0 case (table degeneracy)	
...
***   After-D02 printout in "Al,Mg DPI and POPs.dat". Only in PrFr time:               
...
calculate POPZ(SS) 
      ...
      if(nX.eq.1) write(21, '(f7.4, f8.0, 6e11.4)') tf*1.e9, Te,
     +       POP(PoLeNu(1,1)), POP(PoLeNu(1,2)), POP(PoLeNu(1,3)),	   ! POP of six levels of AL chosen "Params.inp".
     +       POP(PoLeNu(1,4)), POP(PoLeNu(1,5)), POP(PoLeNu(1,6))
     
      if(nX.eq.2) write(22, '(f7.4, f8.0, 6e11.4)') tf*1.e9, Te,
     +       POP(PoLeNu(2,1)), POP(PoLeNu(2,2)), POP(PoLeNu(2,3)),       ! POP of six levels of AL chosen "Params.inp".
...
c Save 1D arrays POP(k) and BE(k) in 3D arrays for next t-step with this "La" and "XE" 
	...
	use mo1             ! two options: "table values (no lowering)", "ion-sphere", depending on "KeReDu" in Params.inp
	implicit none
	real(8) SpheR, DPIsph(HSSm+1)

      if(ng .eq. gpEq(PrFr)) then   ! "ng" in  the NUMBER of "tf"-point in t axis
         write(41,'(/a7, f5.3, a35, i2)') 'tf=', tf*1.d9, 
     +                        'ns is the PrFr time, element number', nX
...
c                                                       Note: only non-AI ELs may be dead. Nucl and AIELs are "never dead" 
	...
	enddo
      ...
      SUBROUTINE SCENARIO(ngw) ! gives R, Te, TiBS, TiD, Den(nXE), %Mg, bp, bc, bw.   
...
*                           gpEq(j) is # of t axis point at which tf=tres(j); j= 1,2,3...6 
      ...
      if(La.ne.2) STOP 'Came in scenario with La =/= 2'    
        ...
        if(ngw .LE. gpEq(1)) then	! i.e. up to t of 1st "print frame"	  
           ...
           R2   = ( Dstrt + frac* ( Dfr(1) - Dstrt ))/2.    ! [cm]  Outer radius of spheric plasma, (La=2)  
           Te   =  TeStrt + frac* (TeFr(1) - TeStrt)        ! [eV]  electron temperature in La=2
           TiD  = (TDstrt + frac* (TDfr(1) - TDstrt))       ! [eV]  Doppler temperature in BS
     ...
     +             (tgp(gpEq(i+1)) -tgp(gpEq(i))) 
            ...
            TiD  = (TDfr(i) + frac* (TDfr(i+1) - TDfr(i)))      ! [keV] to [eV]
  ...
  5     continue
        ...
        if(KeySmooth .GT. 1) then  ! To avoid jumps in Ti(t), exclude jumps in dTe/dt in eq. (3).
c                                    For this, replace broken-line Te(t) with smooth curve 
c                                    consisting of left and right halfs of two Lorentzians 
...
c                                               "gpEq(j)" is t axis point# at which t = tres(j)  	   		          
        ...
        Volu(2)= 1.3333*pin* R2**3    ! [cm3] Volume of plasma sphere 

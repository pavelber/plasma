     ...
     +     nXE= 2              ! number of chemical elements (XE). In present packet of files nX=1 is AL, nX=2 is MG. 
      integer, public :: 
...
c                        = number of t-points in scenario.      
     ...
     +   NTmx = 40 	        ! maximal allowed number of coaxial cylindrical tubes taken for computation of from-sphere SpePow
...
c    +    Uam  = 1.6605402d-24,   ! g; atomic mass unit
     ...
     +  linM(nXE),     ! number of term-to-term lines in [hvmin, hvmax] domain.
     +  PoLeNu(nXE,6), ! For each XE, Six levels chosen for printing their POP(t) in file "POPs_AL" & "POPs_MG". 
     +  gpEq(mSpe),    ! serial numbers of t point at which t = tres(...)
     +  KeRedu,    ! -1 or 0 from "Params.inp". Ionization potential lowering model: none (-1) or IonSphe (0). 
     ...
     +  npa,    ! number of t-points between (tres(j) and tres(j+1) including t= tres(j+1); npa < 49
     +  NT      ! number (<41) of coaxial cylindrical tubes taken for computation of from-sphere SpePow
     ...
     +   SpInEff(LaMx,nvM),            ! [W/cm2/sr/eV] Space- direction- average spectral intensity of radiation in side plasma sphere 
     ...
     +   FWevLor(MNLe), ! Lorentzian (life-time or Stark) part of FWHM [eV] for Voigt shape 
     ...
     +   hvMin, hvMax,  ! edges of full hv interval [eV]; given in Params.inp
     +   hvSmo,         ! edge of [hvMin,hvSmo] domain, where lines have large FWHM (because of W >> A) & do not need fine resolution; comes from Params.inp
     ...
     +   distII,            ! used in functions SigExc, SigInz, SigPhi for avoiding too large values via FAC fit errors.  No corre in REVERSE processes as (1) they are expre via direct & (2) must provide equilibration.             
     +   teiBr, teiE,       ! e-i time of momentum transfer & energy transfer 
     +   TeDot, TePrCO,  
     +   tmPrCO, TeDotPow, Wing,
     +   TiBS, MeanM, dhv, ReverseKKF,    

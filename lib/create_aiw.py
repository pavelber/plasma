...
def copy_for_spectroscopic_numbers(outf, out_dir, spec_numbers, translation_table):
    for n in spec_numbers:
        translation = translation_table[n]
            ...
            with open(in_path, 'rb') as inf:
                for line in inf:
                    ...
                    if len(parts) == 7:
                        ...
                        lev2_search = str(int(parts[0]) + 1)
                        if lev1_search in translation and lev2_search in translation:
                            lev1 = translation[lev1_search]
                            lev2 = translation[lev2_search]
                            outf.write("%5s%5s%5d%5s%15s%15s\n" % (n, lev1, (int(n) + 1), lev2, parts[5], parts[6]))
                        else:
